@section('content')
    <div class="py-12">
        <div class="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 pb-10">
            <p class="text-site dark:text-white text-xl font-semibold pb-2">Aryxo</p>
            <div class="text-siteLite dark:text-gray-300 font-medium flex flex-col md:flex-row flex-wrap">
                <a href="/about" class="pb-2 w-full md:w-1/3 hover:text-gem">About Us</a>
                <a href="/privacy" class="pb-2 w-full md:w-1/3 hover:text-gem">Privacy Policy</a>
                <a href="/terms" class="pb-2 w-full md:w-1/3 hover:text-gem">Terms of Use</a>
                <a href="/legal" class="pb-2 w-full md:w-1/3 hover:text-gem">Legal</a>
                <a href="/sitemap" class="pb-2 w-full md:w-1/3 hover:text-gem">Sitemap</a>
            </div>
        </div>

        <div class="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 pb-10">
            <p class="text-site dark:text-white text-xl font-semibold pb-2">Courses</p>
            <div class="text-siteLite dark:text-gray-300 font-medium flex flex-col md:flex-row flex-wrap">
                <a href="/c" class="pb-2 w-full md:w-1/3 hover:text-gem">C</a>
                <a href="/cpp" class="pb-2 w-full md:w-1/3 hover:text-gem">C++</a>
                <a href="/java" class="pb-2 w-full md:w-1/3 hover:text-gem">Java</a>
                <a href="/python" class="pb-2 w-full md:w-1/3 hover:text-gem">Python</a>
                <a href="/html" class="pb-2 w-full md:w-1/3 hover:text-gem">HTML</a>
                <a href="/css" class="pb-2 w-full md:w-1/3 hover:text-gem">CSS</a>
                <a href="/javascript" class="pb-2 w-full md:w-1/3 hover:text-gem">JS</a>
            </div>
        </div>

        <div class="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 pb-10">
            <p class="text-site dark:text-white text-xl font-semibold pb-2">C</p>
            <div class="text-siteLite dark:text-gray-300 font-medium flex flex-col md:flex-row flex-wrap">
                <a href="/c/introduction-to-c" class="pb-2 w-full md:w-1/3 hover:text-gem">Introduction to C</a>
                <a href="/c/installation-process" class="pb-2 w-full md:w-1/3 hover:text-gem">Installation process</a>
                <a href="/c/running-c-code" class="pb-2 w-full md:w-1/3 hover:text-gem">Running C code</a>
                <a href="/c/keywords-and-identifiers" class="pb-2 w-full md:w-1/3 hover:text-gem">Keywords and Identifiers</a>
                <a href="/c/variables-and-constants" class="pb-2 w-full md:w-1/3 hover:text-gem">Variables and Constants</a>
                <a href="/c/data-types" class="pb-2 w-full md:w-1/3 hover:text-gem">Data Types</a>
                <a href="/c/operators" class="pb-2 w-full md:w-1/3 hover:text-gem">Operators</a>
                <a href="/c/if-and-else" class="pb-2 w-full md:w-1/3 hover:text-gem">If and Else</a>
                <a href="/c/switch-cases" class="pb-2 w-full md:w-1/3 hover:text-gem">Switch Cases</a>
                <a href="/c/for-loop" class="pb-2 w-full md:w-1/3 hover:text-gem">For Loop</a>
                <a href="/c/while-loop" class="pb-2 w-full md:w-1/3 hover:text-gem">While Loop</a>
                <a href="/c/break-and-continue" class="pb-2 w-full md:w-1/3 hover:text-gem">Break and Continue</a>
                <a href="/c/goto" class="pb-2 w-full md:w-1/3 hover:text-gem">Goto</a>
                <a href="/c/fruitless-functions" class="pb-2 w-full md:w-1/3 hover:text-gem">Fruitless Functions</a>
                <a href="/c/function-arguments" class="pb-2 w-full md:w-1/3 hover:text-gem">Function Arguments</a>
                <a href="/c/fruitful-functions" class="pb-2 w-full md:w-1/3 hover:text-gem">Fruitful Functions</a>
                <a href="/c/recursion-storage-class" class="pb-2 w-full md:w-1/3 hover:text-gem">Recursion, Storage Class</a>
                <a href="/c/single-dimensional-array" class="pb-2 w-full md:w-1/3 hover:text-gem">Single Dimensional Array</a>
                <a href="/c/multi-dimensional-array" class="pb-2 w-full md:w-1/3 hover:text-gem">Multi Dimensional Array</a>
                <a href="/c/array-and-functions" class="pb-2 w-full md:w-1/3 hover:text-gem">Array and Functions</a>
                <a href="/c/pointers" class="pb-2 w-full md:w-1/3 hover:text-gem">Pointers</a>
                <a href="/c/functions-and-pointers" class="pb-2 w-full md:w-1/3 hover:text-gem">Functions and Pointers</a>
                <a href="/c/arrays-and-pointers" class="pb-2 w-full md:w-1/3 hover:text-gem">Arrays and Pointers</a>
                <a href="/c/memory-management-in-c" class="pb-2 w-full md:w-1/3 hover:text-gem">Memory Management in C</a>
                <a href="/c/structs" class="pb-2 w-full md:w-1/3 hover:text-gem">Structs</a>
                <a href="/c/structs-and-pointers" class="pb-2 w-full md:w-1/3 hover:text-gem">Structs and Pointers</a>
                <a href="/c/unions" class="pb-2 w-full md:w-1/3 hover:text-gem">Unions</a>
                <a href="/c/unions-and-pointers" class="pb-2 w-full md:w-1/3 hover:text-gem">Unions and Pointers</a>
                <a href="/c/console-input-and-output" class="pb-2 w-full md:w-1/3 hover:text-gem">Console Input and Output</a>
                <a href="/c/file-handling" class="pb-2 w-full md:w-1/3 hover:text-gem">File Handling</a>
                <a href="/c/command-line-arguments" class="pb-2 w-full md:w-1/3 hover:text-gem">Command Line Arguments</a>
                <a href="/c/preprocessor-directives" class="pb-2 w-full md:w-1/3 hover:text-gem">Preprocessor Directives</a>
                <a href="/c/string-handling" class="pb-2 w-full md:w-1/3 hover:text-gem">String Handling</a>
                <a href="/c/math-operations" class="pb-2 w-full md:w-1/3 hover:text-gem">Math Operations</a>
            </div>
        </div>

        <div class="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 pb-10">
            <p class="text-site dark:text-white text-xl font-semibold pb-2">Python</p>
            <div class="text-siteLite dark:text-gray-300 font-medium flex flex-col md:flex-row flex-wrap">
                <a href="/python/python-introduction" class="pb-2 w-full md:w-1/3 hover:text-gem">Python Introduction</a>
                <a href="/python/features-and-applications-of-python" class="pb-2 w-full md:w-1/3 hover:text-gem">Features and Applications of Python</a>
                <a href="/python/python-installation" class="pb-2 w-full md:w-1/3 hover:text-gem">Python Installation</a>
                <a href="/python/keywords-and-identifiers" class="pb-2 w-full md:w-1/3 hover:text-gem">Keywords and Identifiers</a>
                <a href="/python/statements-and-comments" class="pb-2 w-full md:w-1/3 hover:text-gem">Statements and Comments</a>
                <a href="/python/variables" class="pb-2 w-full md:w-1/3 hover:text-gem">Variables</a>
                <a href="/python/operators" class="pb-2 w-full md:w-1/3 hover:text-gem">Operators</a>
                <a href="/python/python-io-and-import" class="pb-2 w-full md:w-1/3 hover:text-gem">Python IO and Import</a>
                <a href="/python/python-namespace" class="pb-2 w-full md:w-1/3 hover:text-gem">Python Namespace</a>
                <a href="/python/if-and-else" class="pb-2 w-full md:w-1/3 hover:text-gem">If and Else</a>
                <a href="/python/loops" class="pb-2 w-full md:w-1/3 hover:text-gem">Loops</a>
                <a href="/python/break-and-continue" class="pb-2 w-full md:w-1/3 hover:text-gem">Break and Continue</a>
                <a href="/python/pass" class="pb-2 w-full md:w-1/3 hover:text-gem">Pass</a>
                <a href="/python/data-types" class="pb-2 w-full md:w-1/3 hover:text-gem">Data Types</a>
                <a href="/python/numbers" class="pb-2 w-full md:w-1/3 hover:text-gem">Numbers</a>
                <a href="/python/string" class="pb-2 w-full md:w-1/3 hover:text-gem">String</a>
                <a href="/python/list" class="pb-2 w-full md:w-1/3 hover:text-gem">List</a>
                <a href="/python/tuple" class="pb-2 w-full md:w-1/3 hover:text-gem">Tuple</a>
                <a href="/python/set" class="pb-2 w-full md:w-1/3 hover:text-gem">Set</a>
                <a href="/python/dictionary" class="pb-2 w-full md:w-1/3 hover:text-gem">Dictionary</a>
                <a href="/python/functions" class="pb-2 w-full md:w-1/3 hover:text-gem">Functions</a>
                <a href="/python/function-arguments" class="pb-2 w-full md:w-1/3 hover:text-gem">Function Arguments</a>
                <a href="/python/python-recursion" class="pb-2 w-full md:w-1/3 hover:text-gem">Python Recursion</a>
                <a href="/python/anonymous-or-lambda-function" class="pb-2 w-full md:w-1/3 hover:text-gem">Anonymous or Lambda Function</a>
                <a href="/python/global-local-and-nonlocal-functions" class="pb-2 w-full md:w-1/3 hover:text-gem">Global Local and Nonlocal Functions</a>
                <a href="/python/global-keyword" class="pb-2 w-full md:w-1/3 hover:text-gem">Global Keyword</a>
                <a href="/python/python-modules" class="pb-2 w-full md:w-1/3 hover:text-gem">Python Modules</a>
                <a href="/python/datetime-module" class="pb-2 w-full md:w-1/3 hover:text-gem">Datetime Module</a>
                <a href="/python/math-module" class="pb-2 w-full md:w-1/3 hover:text-gem">Math Module</a>
                <a href="/python/os-module" class="pb-2 w-full md:w-1/3 hover:text-gem">OS Module</a>
                <a href="/python/sys-module" class="pb-2 w-full md:w-1/3 hover:text-gem">Sys Module</a>
                <a href="/python/python-packages" class="pb-2 w-full md:w-1/3 hover:text-gem">Python Packages</a>
                <a href="/python/python-pip" class="pb-2 w-full md:w-1/3 hover:text-gem">Python PIP</a>
                <a href="/python/python-json" class="pb-2 w-full md:w-1/3 hover:text-gem">Python JSON</a>
                <a href="/python/python-regex" class="pb-2 w-full md:w-1/3 hover:text-gem">Python RegEx</a>
                <a href="/python/exception-and-its-types" class="pb-2 w-full md:w-1/3 hover:text-gem">Exception and its Types</a>
                <a href="/python/exception-handling" class="pb-2 w-full md:w-1/3 hover:text-gem">Exception Handling</a>
                <a href="/python/file-handling" class="pb-2 w-full md:w-1/3 hover:text-gem">File Handling</a>
                <a href="/python/python-directory" class="pb-2 w-full md:w-1/3 hover:text-gem">Python Directory</a>
                <a href="/python/arrays" class="pb-2 w-full md:w-1/3 hover:text-gem">Arrays</a>
                <a href="/python/array-operations" class="pb-2 w-full md:w-1/3 hover:text-gem">Array Operations</a>
                <a href="/python/oops-concepts" class="pb-2 w-full md:w-1/3 hover:text-gem">OOPs Concepts</a>
                <a href="/python/class-and-object" class="pb-2 w-full md:w-1/3 hover:text-gem">Class and Object</a>
                <a href="/python/constructors" class="pb-2 w-full md:w-1/3 hover:text-gem">Constructors</a>
                <a href="/python/inheritance" class="pb-2 w-full md:w-1/3 hover:text-gem">Inheritance </a>
                <a href="/python/abstraction" class="pb-2 w-full md:w-1/3 hover:text-gem">Abstraction</a>
                <a href="/python/operator-overloading" class="pb-2 w-full md:w-1/3 hover:text-gem">Operator Overloading</a>
            </div>
        </div>

        <div class="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 pb-10">
            <p class="text-site dark:text-white text-xl font-semibold pb-2">HTML</p>
            <div class="text-siteLite dark:text-gray-300 font-medium flex flex-col md:flex-row flex-wrap">
                <a href="/html/introduction-to-html" class="pb-2 w-full md:w-1/3 hover:text-gem">Introduction to HTML</a>
                <a href="/html/editors" class="pb-2 w-full md:w-1/3 hover:text-gem">Editors</a>
                <a href="/html/basic-tags" class="pb-2 w-full md:w-1/3 hover:text-gem">Basic Tags</a>
                <a href="/html/document-type" class="pb-2 w-full md:w-1/3 hover:text-gem">Document Type</a>
                <a href="/html/elements" class="pb-2 w-full md:w-1/3 hover:text-gem">Elements</a>
                <a href="/html/core-attributes" class="pb-2 w-full md:w-1/3 hover:text-gem">Core Attributes</a>
                <a href="/html/special-attributes" class="pb-2 w-full md:w-1/3 hover:text-gem">Special Attributes</a>
                <a href="/html/generic-attributes" class="pb-2 w-full md:w-1/3 hover:text-gem">Generic Attributes</a>
                <a href="/html/style" class="pb-2 w-full md:w-1/3 hover:text-gem">Style</a>
                <a href="/html/fonts" class="pb-2 w-full md:w-1/3 hover:text-gem">Fonts</a>
                <a href="/html/text-size" class="pb-2 w-full md:w-1/3 hover:text-gem">Text Size</a>
                <a href="/html/text-alignment" class="pb-2 w-full md:w-1/3 hover:text-gem">Text Alignment</a>
                <a href="/html/background-color" class="pb-2 w-full md:w-1/3 hover:text-gem">Background Color</a>
                <a href="/html/text-color" class="pb-2 w-full md:w-1/3 hover:text-gem">Text Color</a>
                <a href="/html/bold" class="pb-2 w-full md:w-1/3 hover:text-gem">Bold</a>
                <a href="/html/italic" class="pb-2 w-full md:w-1/3 hover:text-gem">Italic</a>
                <a href="/html/underline" class="pb-2 w-full md:w-1/3 hover:text-gem">Underline</a>
                <a href="/html/strike" class="pb-2 w-full md:w-1/3 hover:text-gem">Strike</a>
                <a href="/html/insert" class="pb-2 w-full md:w-1/3 hover:text-gem">Insert</a>
                <a href="/html/delete" class="pb-2 w-full md:w-1/3 hover:text-gem">Delete</a>
                <a href="/html/highlighting" class="pb-2 w-full md:w-1/3 hover:text-gem">Highlighting</a>
                <a href="/html/subscript-and-superscript" class="pb-2 w-full md:w-1/3 hover:text-gem">Subscript and Superscript</a>
                <a href="/html/abbreviation" class="pb-2 w-full md:w-1/3 hover:text-gem">Abbreviation</a>
                <a href="/html/larger-and-smaller-text" class="pb-2 w-full md:w-1/3 hover:text-gem">Larger and Smaller text</a>
                <a href="/html/text-links" class="pb-2 w-full md:w-1/3 hover:text-gem">Text Links</a>
                <a href="/html/image-links" class="pb-2 w-full md:w-1/3 hover:text-gem">Image Links</a>
                <a href="/html/email-links" class="pb-2 w-full md:w-1/3 hover:text-gem">Email Links</a>
                <a href="/html/ordered" class="pb-2 w-full md:w-1/3 hover:text-gem">Ordered</a>
                <a href="/html/unordered" class="pb-2 w-full md:w-1/3 hover:text-gem">Unordered</a>
                <a href="/html/nested" class="pb-2 w-full md:w-1/3 hover:text-gem">Nested</a>
                <a href="/html/description" class="pb-2 w-full md:w-1/3 hover:text-gem">Description</a>
                <a href="/html/table" class="pb-2 w-full md:w-1/3 hover:text-gem">Table</a>
                <a href="/html/table-sizes" class="pb-2 w-full md:w-1/3 hover:text-gem">Table Sizes </a>
                <a href="/html/table-headers" class="pb-2 w-full md:w-1/3 hover:text-gem">Table Headers</a>
                <a href="/html/paddings-and-spacing" class="pb-2 w-full md:w-1/3 hover:text-gem">Paddings and Spacing</a>
                <a href="/html/colspan-and-rowspan" class="pb-2 w-full md:w-1/3 hover:text-gem">Colspan and Rowspan</a>
                <a href="/html/table-borders" class="pb-2 w-full md:w-1/3 hover:text-gem">Table Borders</a>
                <a href="/html/inserting-images" class="pb-2 w-full md:w-1/3 hover:text-gem">Inserting Images</a>
                <a href="/html/image-map" class="pb-2 w-full md:w-1/3 hover:text-gem">Image Map</a>
                <a href="/html/background-images" class="pb-2 w-full md:w-1/3 hover:text-gem">Background Images</a>
                <a href="/html/inline-style-sheets" class="pb-2 w-full md:w-1/3 hover:text-gem">Inline Style Sheets</a>
                <a href="/html/embedded-style-sheets" class="pb-2 w-full md:w-1/3 hover:text-gem">Embedded Style Sheets</a>
                <a href="/html/external-style-sheets" class="pb-2 w-full md:w-1/3 hover:text-gem">External Style Sheets</a>
                <a href="/html/class-attribute" class="pb-2 w-full md:w-1/3 hover:text-gem">Class Attribute</a>
                <a href="/html/multiple-classes" class="pb-2 w-full md:w-1/3 hover:text-gem">Multiple Classes</a>
                <a href="/html/id-attribute" class="pb-2 w-full md:w-1/3 hover:text-gem">Id Attribute</a>
                <a href="/html/text" class="pb-2 w-full md:w-1/3 hover:text-gem">Text</a>
                <a href="/html/color" class="pb-2 w-full md:w-1/3 hover:text-gem">Color</a>
                <a href="/html/button" class="pb-2 w-full md:w-1/3 hover:text-gem">Button</a>
                <a href="/html/checkbox-and-radio-buttons" class="pb-2 w-full md:w-1/3 hover:text-gem">Checkbox and Radio Buttons</a>
                <a href="/html/input-validation" class="pb-2 w-full md:w-1/3 hover:text-gem">Input Validation</a>
                <a href="/html/password" class="pb-2 w-full md:w-1/3 hover:text-gem">Password</a>
                <a href="/html/file" class="pb-2 w-full md:w-1/3 hover:text-gem">File</a>
                <a href="/html/reset" class="pb-2 w-full md:w-1/3 hover:text-gem">Reset</a>
                <a href="/html/submit" class="pb-2 w-full md:w-1/3 hover:text-gem">Submit</a>
                <a href="/html/number" class="pb-2 w-full md:w-1/3 hover:text-gem">Number</a>
                <a href="/html/range" class="pb-2 w-full md:w-1/3 hover:text-gem">Range</a>
                <a href="/html/search" class="pb-2 w-full md:w-1/3 hover:text-gem">Search</a>
                <a href="/html/date-and-time" class="pb-2 w-full md:w-1/3 hover:text-gem">Date and Time</a>
                <a href="/html/form-attributes" class="pb-2 w-full md:w-1/3 hover:text-gem">Form Attributes</a>
                <a href="/html/form-elements" class="pb-2 w-full md:w-1/3 hover:text-gem">Form Elements</a>
                <a href="/html/input-types-and-attributes" class="pb-2 w-full md:w-1/3 hover:text-gem">Input Types and Attributes</a>
                <a href="/html/iframe" class="pb-2 w-full md:w-1/3 hover:text-gem">IFrame</a>
                <a href="/html/setting-the-frame-size" class="pb-2 w-full md:w-1/3 hover:text-gem">Setting the Frame Size</a>
                <a href="/html/iframe-as-link-target" class="pb-2 w-full md:w-1/3 hover:text-gem">IFrame as Link Target</a>
            </div>
        </div>

    </div>
@endsection
